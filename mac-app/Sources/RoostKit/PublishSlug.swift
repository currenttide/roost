...
/// Pure helpers for the Publish verb (server `POST /publish?name=`). The control
/// plane is authoritative — it slugifies `?name=` and 400s an unfixable name — but
/// we derive a sensible default slug from the picked file and pre-validate so the
/// menu-bar app gives instant feedback instead of a round-trip. Mirrors
/// `roost/publish.py::normalize_slug` (`strip().lower().replace(" ", "-")` →
/// `^[a-z0-9][a-z0-9-]{0,39}$`) and the iOS app's `PublishSlug`. Foundation-only,
/// so the Linux harness covers it.
public enum PublishSlug {
    /// The server's slug grammar: lowercase alnum start, then alnum/hyphen, ≤40.
    /// Pinned byte-for-byte to `roost/publish.py::_SLUG_RE`.
    public static let pattern = "^[a-z0-9][a-z0-9-]{0,39}$"

    /// Apply the server's normalization (trim, lowercase, spaces→`-`) WITHOUT
    /// validating. Used to live-preview the slug the server will store as the user
    ...
    public static func suggestion(fromFilename filename: String) -> String {
        var stem = filename
...
/// one-shot publish endpoint 400s a body that isn't a valid `tar.gz`; this lets
/// the app reject an obviously-wrong pick (e.g. a plain folder or a `.zip`) before
/// the upload. It is a cheap necessary check, not a full tar.gz validation — the
/// server still has the final say. Mirrors the iOS `BundleCheck`.
