...
object Parsers {
    ...
    private fun JSONObject.asPlainMap(): Map<String, Any?> {
        val out = LinkedHashMap<String, Any?>()
        for (k in keys()) {
            out[k] = when (val v = opt(k)) {
                JSONObject.NULL -> null
                is JSONObject -> v.asPlainMap()
                is JSONArray -> v.asPlainList()
                else -> v
            }
        }
        return out
    ...
    private fun JSONArray.asPlainList(): List<Any?> =
    ...
    fun parseJob(o: JSONObject): Job {
        ...
        return Job(
            ...
            specRequires = spec?.optJSONObject("requires")?.asPlainMap()
                ?: o.optJSONObject("requires")?.asPlainMap()
                ?: emptyMap(),
        )
    }
