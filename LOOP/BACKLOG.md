...
### R114. Auth-disabled CP silently accepts worker-plane requests — loud guard — `done` *(2026-06-09, PR #128 — ROOST_INSECURE_NO_AUTH=1 + --insecure opt-in, refusal on non-loopback bind, !!! banner, docs; a partial guard existed but referenced a flag that didn't exist)* `human-promoted`
...
### R115. Sweeper exceptions swallowed — log + surface a failure counter — `done` *(2026-06-09, PR #129 — _note_sweep_failure deduped logging + roost_sweep_failures_total{phase} across all 7 periodic phases; the swallows were in _sweep_loop ~4115, not ~2258 which re-raises correctly)* `human-promoted`
...
### R116. Assignment scan unbounded — LIMIT with provably-unchanged semantics — `done` *(2026-06-09, PR #130 — seek-paginated batches of 200 in the old visit order; same winner for any queue size; naive-LIMIT ghost-window case proven by test; worst case stays O(queue) by design, documented)* `human-promoted`
...
### R117. Steward timeout/failure invisible — structured signal — `done` *(2026-06-09, PR #126 — outcome labels ok|no-binary|spawn-failure|timeout|bad-output, STEWARD_AGENT_FAILED log, steward_failures/steward_last_error heartbeat caps only-when-nonzero; heuristic fallback unchanged)* `human-promoted`
...
### R118. Recovery-path tests: CP restart over stale leases + concurrent-assignment race — `done` *(2026-06-09, PR #127 — 4 tests, zero production changes, zero bugs found: restart/WAL handoff, R19/R52 attempt accounting, decliner persistence, BEGIN IMMEDIATE exactly-once all hold)* `human-promoted`
